...
function test_endpoints {
  wget --quiet --header "Host: openfisca.${1}mes-aides.gouv.fr" --output-document=- 0.0.0.0/variable/parisien | grep --silent openfisca_paris/paris.py
  wget --quiet --header "Host: monitor.${1}mes-aides.gouv.fr" --content-on-error --output-document=- 0.0.0.0 | grep --silent $CIRCLE_SHA1
}
